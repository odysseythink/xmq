...
// IsInconsistent checks for cases where an unexpected number of xmqd connections are
// reporting the same information to xmqlookupd (ie: multiple instances are using the
// same broadcast address), or cases where some xmqd are not reporting to all xmqlookupd.
