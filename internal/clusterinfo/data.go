...
import (
	...
	"mlib.com/mlog"
	"mlib.com/mrun/semver"
	"mlib.com/xmq/internal/http_api"
	"mlib.com/xmq/internal/stringy"
)
...
func New(client *http_api.Client) *ClusterInfo {
	return &ClusterInfo{
		client: client,
	}
}
...
func (c *ClusterInfo) GetLookupdTopics(lookupdHTTPAddrs []string) ([]string, error) {
	...
	for _, addr := range lookupdHTTPAddrs {
		...
		go func(addr string) {
			...
			mlog.Infof("CI: querying xmqlookupd %s", endpoint)
	...
	if len(errs) == len(lookupdHTTPAddrs) {
		return nil, fmt.Errorf("failed to query any xmqlookupd: %s", ErrList(errs))
	}
...
// GetLookupdProducers returns Producers of all the xmqd connected to the given lookupds
...
// GetLookupdTopicProducers returns Producers of all the xmqd for a given topic by
...
// GetXMQDTopics returns a []string containing all the topics produced by the given xmqd
func (c *ClusterInfo) GetXMQDTopics(xmqdHTTPAddrs []string) ([]string, error) {
	var topics []string
	var lock sync.Mutex
	var wg sync.WaitGroup
	var errs []error
	...
	for _, addr := range xmqdHTTPAddrs {
		wg.Add(1)
		go func(addr string) {
			defer wg.Done()

			endpoint := fmt.Sprintf("http://%s/stats?format=json", addr)
			mlog.Infof("CI: querying xmqd %s", endpoint)
	...
	if len(errs) == len(xmqdHTTPAddrs) {
		return nil, fmt.Errorf("failed to query any xmqd: %s", ErrList(errs))
	}
...
// GetXMQDProducers returns Producers of all the given xmqd
func (c *ClusterInfo) GetXMQDProducers(xmqdHTTPAddrs []string) (Producers, error) {
	var producers Producers
	var lock sync.Mutex
	var wg sync.WaitGroup
	var errs []error
...
// GetXMQDTopicProducers returns Producers containing the addresses of all the xmqd
// that produce the given topic
func (c *ClusterInfo) GetXMQDTopicProducers(topic string, xmqdHTTPAddrs []string) (Producers, error) {
	var producers Producers
	var lock sync.Mutex
	var wg sync.WaitGroup
	var errs []error
...
// GetXMQDStats returns aggregate topic and channel stats from the given Producers
//
...
func (c *ClusterInfo) GetXMQDStats(producers Producers,
	...
	if len(errs) == len(producers) {
		return nil, nil, fmt.Errorf("failed to query any xmqd: %s", ErrList(errs))
	}
...
// TombstoneNodeForTopic tombstones the given node for the given topic on all the given xmqlookupd
...
func (c *ClusterInfo) TombstoneNodeForTopic(topic string, node string, lookupdHTTPAddrs []string) error {
	...
	err := c.xmqlookupdPOST(lookupdHTTPAddrs, "topic/tombstone", qs)
	if err != nil {
	...
	producers, err := c.GetXMQDProducers([]string{node})
	if err != nil {
		pe, ok := err.(PartialErr)
		if !ok {
			return err
...
func (c *ClusterInfo) CreateTopicChannel(topicName string, channelName string, lookupdHTTPAddrs []string) error {
	...
	// create the topic on all the xmqlookupd
	qs := fmt.Sprintf("topic=%s", url.QueryEscape(topicName))
	err := c.xmqlookupdPOST(lookupdHTTPAddrs, "topic/create", qs)
	if err != nil {
		pe, ok := err.(PartialErr)
		if !ok {
			return err
	...
	if len(channelName) > 0 {
		...
		// create the channel on all the xmqlookupd
		err := c.xmqlookupdPOST(lookupdHTTPAddrs, "channel/create", qs)
		if err != nil {
			pe, ok := err.(PartialErr)
			if !ok {
				return err
		...
		// create the channel on all the xmqd that produce the topic
...
func (c *ClusterInfo) DeleteTopic(topicName string, lookupdHTTPAddrs []string, xmqdHTTPAddrs []string) error {
	var errs []error

	// for topic removal, you need to get all the producers _first_
	producers, err := c.GetTopicProducers(topicName, lookupdHTTPAddrs, xmqdHTTPAddrs)
	if err != nil {
		pe, ok := err.(PartialErr)
		if !ok {
			return err
	...
	// remove the topic from all the xmqlookupd
	err = c.xmqlookupdPOST(lookupdHTTPAddrs, "topic/delete", qs)
	if err != nil {
		pe, ok := err.(PartialErr)
		if !ok {
			return err
	...
	// remove the topic from all the xmqd that produce this topic
	err = c.producersPOST(producers, "topic/delete", qs)
	if err != nil {
		pe, ok := err.(PartialErr)
		if !ok {
...
func (c *ClusterInfo) DeleteChannel(topicName string, channelName string, lookupdHTTPAddrs []string, xmqdHTTPAddrs []string) error {
	var errs []error
	...
	// remove the channel from all the xmqlookupd
	err = c.xmqlookupdPOST(lookupdHTTPAddrs, "channel/delete", qs)
	if err != nil {
		pe, ok := err.(PartialErr)
		if !ok {
			return err
	...
	// remove the channel from all the xmqd that produce this topic
...
func (c *ClusterInfo) PauseTopic(topicName string, lookupdHTTPAddrs []string, xmqdHTTPAddrs []string) error {
	qs := fmt.Sprintf("topic=%s", url.QueryEscape(topicName))
	return c.actionHelper(topicName, lookupdHTTPAddrs, xmqdHTTPAddrs, "topic/pause", qs)
}

func (c *ClusterInfo) UnPauseTopic(topicName string, lookupdHTTPAddrs []string, xmqdHTTPAddrs []string) error {
	qs := fmt.Sprintf("topic=%s", url.QueryEscape(topicName))
	return c.actionHelper(topicName, lookupdHTTPAddrs, xmqdHTTPAddrs, "topic/unpause", qs)
}

func (c *ClusterInfo) PauseChannel(topicName string, channelName string, lookupdHTTPAddrs []string, xmqdHTTPAddrs []string) error {
	qs := fmt.Sprintf("topic=%s&channel=%s", url.QueryEscape(topicName), url.QueryEscape(channelName))
	return c.actionHelper(topicName, lookupdHTTPAddrs, xmqdHTTPAddrs, "channel/pause", qs)
}

func (c *ClusterInfo) UnPauseChannel(topicName string, channelName string, lookupdHTTPAddrs []string, xmqdHTTPAddrs []string) error {
	qs := fmt.Sprintf("topic=%s&channel=%s", url.QueryEscape(topicName), url.QueryEscape(channelName))
	return c.actionHelper(topicName, lookupdHTTPAddrs, xmqdHTTPAddrs, "channel/unpause", qs)
}

func (c *ClusterInfo) EmptyTopic(topicName string, lookupdHTTPAddrs []string, xmqdHTTPAddrs []string) error {
	qs := fmt.Sprintf("topic=%s", url.QueryEscape(topicName))
	return c.actionHelper(topicName, lookupdHTTPAddrs, xmqdHTTPAddrs, "topic/empty", qs)
}

func (c *ClusterInfo) EmptyChannel(topicName string, channelName string, lookupdHTTPAddrs []string, xmqdHTTPAddrs []string) error {
	qs := fmt.Sprintf("topic=%s&channel=%s", url.QueryEscape(topicName), url.QueryEscape(channelName))
	return c.actionHelper(topicName, lookupdHTTPAddrs, xmqdHTTPAddrs, "channel/empty", qs)
}

func (c *ClusterInfo) actionHelper(topicName string, lookupdHTTPAddrs []string, xmqdHTTPAddrs []string, uri string, qs string) error {
	var errs []error
...
func (c *ClusterInfo) GetProducers(lookupdHTTPAddrs []string, xmqdHTTPAddrs []string) (Producers, error) {
	if len(lookupdHTTPAddrs) != 0 {
		return c.GetLookupdProducers(lookupdHTTPAddrs)
	}
	return c.GetXMQDProducers(xmqdHTTPAddrs)
}

func (c *ClusterInfo) GetTopicProducers(topicName string, lookupdHTTPAddrs []string, xmqdHTTPAddrs []string) (Producers, error) {
	if len(lookupdHTTPAddrs) != 0 {
		return c.GetLookupdTopicProducers(topicName, lookupdHTTPAddrs)
	}
	return c.GetXMQDTopicProducers(topicName, xmqdHTTPAddrs)
}

func (c *ClusterInfo) xmqlookupdPOST(addrs []string, uri string, qs string) error {
	var errs []error
