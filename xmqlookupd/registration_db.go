...
func (p *Producer) String() string {
	return fmt.Sprintf("%s [%d, %d]", p.peerInfo.BroadcastAddress, p.peerInfo.PeerInfo.TcpPort, p.peerInfo.PeerInfo.HttpPort)
}
...
func (r *RegistrationDB) AddProducer(k Registration, p *Producer) bool {
	...
	_, found := producers[p.peerInfo.PeerInfo.Id]
	if !found {
		producers[p.peerInfo.PeerInfo.Id] = p
	}
	return !found
}
...
func (r *RegistrationDB) FindProducers(category string, key string, subkey string) Producers {
	...
	for k, producers := range r.registrationMap {
		...
		for _, producer := range producers {
			_, found := results[producer.peerInfo.PeerInfo.Id]
			if !found {
				results[producer.peerInfo.PeerInfo.Id] = struct{}{}
