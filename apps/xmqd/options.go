...
func (cfg config) Validate() error {
	...
	if v, exists := cfg["tls_required"]; exists {
		...
		if err == nil {
			...
			mlog.Errorf("failed parsing tls_required %+v", v)
			return fmt.Errorf("failed parsing tls_required %+v", v)
		}
	}
	if v, exists := cfg["tls_min_version"]; exists {
		...
		if err == nil {
			...
			mlog.Errorf("failed parsing tls_min_version %+v", v)
			return fmt.Errorf("failed parsing tls_min_version %+v", v)
		}
	}
...
func xmqdFlagSet(opts *xmqd.Options) *flag.FlagSet {
	flagSet := flag.NewFlagSet("xmqd", flag.ExitOnError)
	...
	flagSet.String("log-prefix", "", "log message prefix")
	flagSet.Bool("verbose", false, "[deprecated] has no effect, use --log-level")

	flagSet.String("node-id", opts.ID, "unique part for message IDs, (default is uuid)")
	...
	flagSet.String("raft-address", opts.RaftAddress, "address to listen on for raft clients (<addr>:<port> for TCP/IP or <path> for unix socket)")
	clusterRaftAddrs := app.StringArray{}
	flagSet.Var(&clusterRaftAddrs, "cluster-raft-address", "cluster raft TCP address (may be given multiple times)(<addr>:<port> for TCP/IP or <path> for unix socket)")
	...
	flagSet.Int("broadcast-tcp-port", opts.BroadcastTCPPort, "TCP port that will be registered with lookupd (defaults to the TCP port that this xmqd is listening on)")
	flagSet.Int("broadcast-http-port", opts.BroadcastHTTPPort, "HTTP port that will be registered with lookupd (defaults to the HTTP port that this xmqd is listening on)")
	lookupdGrpcAddrs := app.StringArray{}
	flagSet.Var(&lookupdGrpcAddrs, "lookupd-grpc-address", "lookupd gprc address (may be given multiple times)")
	...
	flagSet.Int("max-channel-consumers", opts.MaxChannelConsumers, "maximum channel consumer connection count per xmqd instance (default 0, i.e., unlimited)")
	...
	flagSet.Int("max-deflate-level", opts.MaxDeflateLevel, "max deflate compression level a client can negotiate (> values == > xmqd CPU usage)")
