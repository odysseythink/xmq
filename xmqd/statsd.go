...
func (n *XMQD) statsdLoop() {
	...
	for {
		...
		case <-ticker.C:
			...
			if err != nil {
				mlog.Errorf("failed to create UDP socket to statsd(%s)", addr)
				continue
			}
			sw := writers.NewSpreadWriter(conn, interval-time.Second, n.ctx)
			bw := writers.NewBoundaryBufferedWriter(sw, n.getOpts().StatsdUDPPacketSize)
			client := statsd.NewClient(bw, prefix)

			mlog.Infof("STATSD: pushing stats to %s", addr)
