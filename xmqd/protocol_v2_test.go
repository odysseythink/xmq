...
func mustStartXMQD(opts *Options) (net.Addr, net.Addr, *XMQD) {
...
func mustConnectXMQD(tcpAddr net.Addr) (net.Conn, error) {
...
func sub(t *testing.T, conn io.ReadWriter, topicName string, channelName string) {
	_, err := xmq.Subscribe(topicName, channelName).WriteTo(conn)
	test.Nil(t, err)
...
func authCmd(t *testing.T, conn io.ReadWriter, authSecret string, expectSuccess string) {
	auth, _ := xmq.Auth(authSecret)
	_, err := auth.WriteTo(conn)
	test.Nil(t, err)
	if expectSuccess != "" {
		readValidate(t, conn, xmq.FrameTypeResponse, expectSuccess)
	}
}
...
func subFail(t *testing.T, conn io.ReadWriter, topicName string, channelName string) {
	...
	frameType, _, _ := xmq.UnpackResponse(resp)
