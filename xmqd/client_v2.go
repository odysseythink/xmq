...
func newClientV2(id string, conn net.Conn, xmqd *XMQD) *clientV2 {
	...
	c := &clientV2{
		...
		OutputBufferTimeout: xmqd.getOpts().OutputBufferTimeout,

		MsgTimeout: xmqd.getOpts().MsgTimeout,
		...
		HeartbeatInterval: xmqd.getOpts().ClientTimeout / 2,
...
func (c *clientV2) Identify(data identifyDataV2) error {
	mlog.Infof("[%s] IDENTIFY: %+v", c, data)
...
func (c *clientV2) IsReadyForMessages() bool {
	...
	mlog.Debugf("[%s] state rdy: %4d inflt: %4d", c, readyCount, inFlightCount)
...
func (c *clientV2) SetHeartbeatInterval(desiredInterval int) error {
	...
	case desiredInterval >= 1000 &&
		desiredInterval <= int(c.xmqd.getOpts().MaxHeartbeatInterval/time.Millisecond):
...
func (c *clientV2) SetOutputBuffer(desiredSize int, desiredTimeout int) error {
	...
	case desiredTimeout == 0:
		// do nothing (use default)
	case true &&
		desiredTimeout >= int(c.xmqd.getOpts().MinOutputBufferTimeout/time.Millisecond) &&
		desiredTimeout <= int(c.xmqd.getOpts().MaxOutputBufferTimeout/time.Millisecond):
	...
	case desiredSize == 0:
		// do nothing (use default)
	case desiredSize >= 64 && desiredSize <= int(c.xmqd.getOpts().MaxOutputBufferSize):
...
func (c *clientV2) SetMsgTimeout(msgTimeout int) error {
	...
	case msgTimeout >= 1000 &&
		msgTimeout <= int(c.xmqd.getOpts().MaxMsgTimeout/time.Millisecond):
...
func (c *clientV2) UpgradeTLS() error {
	...
	tlsConn := tls.Server(c.Conn, c.xmqd.tlsConfig)
...
func (c *clientV2) QueryAuthd() error {
	...
	authState, err := auth.QueryAnyAuthd(c.xmqd.getOpts().AuthHTTPAddresses,
		remoteIP, tlsEnabled, commonName, c.AuthSecret,
		c.xmqd.clientTLSConfig,
		c.xmqd.getOpts().HTTPClientConnectTimeout,
		c.xmqd.getOpts().HTTPClientRequestTimeout)
	if err != nil {
		return err
	}
