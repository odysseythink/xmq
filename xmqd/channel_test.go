...
func TestChannelEmptyConsumer(t *testing.T) {
	...
	conn, _ := mustConnectXMQD(tcpAddr)
	defer conn.Close()
	...
	channel := topic.GetChannel("channel")
	clientID := uuid.NewV4().String()
	client := newClientV2(clientID, conn, xmqd)
...
func TestMaxChannelConsumers(t *testing.T) {
	...
	client1 := newClientV2(uuid.NewV4().String(), conn, xmqd)
	client1.SetReadyCount(25)
	err := channel.AddClient(client1.ID, client1)
	test.Equal(t, err, nil)

	client2 := newClientV2(uuid.NewV4().String(), conn, xmqd)
