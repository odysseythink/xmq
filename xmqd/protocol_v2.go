...
func (p *protocolV2) NewClient(conn net.Conn) protocol.Client {
	// clientID := atomic.AddInt64(&p.xmqd.clientIDSequence, 1)
	clientID := uuid.NewV4().String()
	return newClientV2(clientID, conn, p.xmqd)
}
...
func (p *protocolV2) IOLoop(c protocol.Client) error {
	...
	for {
		...
		mlog.Debugf("PROTOCOL(V2): [%s] %s", client, params)
		...
		if err != nil {
			...
			mlog.Errorf("[%s] - %s%s", client, err, ctx)

			sendErr := p.Send(client, frameTypeError, []byte(err.Error()))
			if sendErr != nil {
				mlog.Errorf("[%s] - %s%s", client, sendErr, ctx)
				break
			}

			// errors of type FatalClientErr should forceably close the connection
			if _, ok := err.(*protocol.FatalClientErr); ok {
				mlog.Errorf("Send FatalClientErr:%v", err)
				break
			}
			continue
		}
		...
		if response != nil {
			...
			if err != nil {
				mlog.Errorf("failed to send response - %v", err)
	...
	mlog.Infof("PROTOCOL(V2): [%s] exiting ioloop", client)
...
func (p *protocolV2) SendMessage(client *clientV2, msg *Message) error {
	mlog.Debugf("PROTOCOL(V2): writing msg(%s) to client(%s) - %s", msg.ID, client, msg.Body)
...
func (p *protocolV2) messagePump(client *clientV2, startedChan chan bool) {
	...
	for {
		...
		case subChannel = <-subEventChan:
			mlog.Debugf("----subChannel=%#v", subChannel)
...
exit:
	mlog.Infof("PROTOCOL(V2): [%s] exiting messagePump", client)
	heartbeatTicker.Stop()
	outputBufferTicker.Stop()
	if err != nil {
		mlog.Errorf("PROTOCOL(V2): [%s] messagePump error - %s", client, err)
	}
}
...
func (p *protocolV2) IDENTIFY(client *clientV2, params [][]byte) ([]byte, error) {
	...
	if int64(bodyLen) > p.xmqd.getOpts().MaxBodySize {
		return nil, protocol.NewFatalClientErr(nil, "E_BAD_BODY",
			fmt.Sprintf("IDENTIFY body too big %d > %d", bodyLen, p.xmqd.getOpts().MaxBodySize))
	}
	...
	mlog.Debugf("PROTOCOL(V2): [%s] %+v", client, identifyData)
	...
	tlsv1 := p.xmqd.tlsConfig != nil && identifyData.TLSv1
	deflate := p.xmqd.getOpts().DeflateEnabled && identifyData.Deflate
	deflateLevel := 6
	if deflate && identifyData.DeflateLevel > 0 {
		deflateLevel = identifyData.DeflateLevel
	}
	if max := p.xmqd.getOpts().MaxDeflateLevel; max < deflateLevel {
		deflateLevel = max
	}
	snappy := p.xmqd.getOpts().SnappyEnabled && identifyData.Snappy
	...
	resp, err := json.Marshal(struct {
		...
		MaxRdyCount:         p.xmqd.getOpts().MaxRdyCount,
		Version:             version.Binary,
		MaxMsgTimeout:       int64(p.xmqd.getOpts().MaxMsgTimeout / time.Millisecond),
		MsgTimeout:          int64(client.MsgTimeout / time.Millisecond),
		TLSv1:               tlsv1,
		Deflate:             deflate,
		DeflateLevel:        deflateLevel,
		MaxDeflateLevel:     p.xmqd.getOpts().MaxDeflateLevel,
		Snappy:              snappy,
		SampleRate:          client.SampleRate,
		AuthRequired:        p.xmqd.IsAuthEnabled(),
	...
	if tlsv1 {
		mlog.Infof("PROTOCOL(V2): [%s] upgrading connection to TLS", client)
	...
	if snappy {
		mlog.Infof("PROTOCOL(V2): [%s] upgrading connection to snappy", client)
	...
	if deflate {
		mlog.Infof("PROTOCOL(V2): [%s] upgrading connection to deflate (level %d)", client, deflateLevel)
...
func (p *protocolV2) AUTH(client *clientV2, params [][]byte) ([]byte, error) {
	...
	if int64(bodyLen) > p.xmqd.getOpts().MaxBodySize {
		return nil, protocol.NewFatalClientErr(nil, "E_BAD_BODY",
			fmt.Sprintf("AUTH body too big %d > %d", bodyLen, p.xmqd.getOpts().MaxBodySize))
	}
	...
	if err := client.Auth(string(body)); err != nil {
		...
		mlog.Warningf("PROTOCOL(V2): [%s] AUTH failed %s", client, err)
...
func (p *protocolV2) SUB(client *clientV2, params [][]byte) ([]byte, error) {
	...
	var channel *Channel
	for i := 1; ; i++ {
		topic := p.xmqd.cluster.GetTopic(topicName)
...
func (p *protocolV2) RDY(client *clientV2, params [][]byte) ([]byte, error) {
	...
	if count < 0 || count > p.xmqd.getOpts().MaxRdyCount {
		// this needs to be a fatal error otherwise clients would have
		// inconsistent state
		return nil, protocol.NewFatalClientErr(nil, "E_INVALID",
			fmt.Sprintf("RDY count %d out of range 0-%d", count, p.xmqd.getOpts().MaxRdyCount))
	}
...
func (p *protocolV2) REQ(client *clientV2, params [][]byte) ([]byte, error) {
	...
	maxReqTimeout := p.xmqd.getOpts().MaxReqTimeout
	...
	if clampedTimeout != timeoutDuration {
		mlog.Infof("PROTOCOL(V2): [%s] REQ timeout %d out of range 0-%d. Setting to %d",
...
func (p *protocolV2) PUB(client *clientV2, params [][]byte) ([]byte, error) {
	...
	if int64(bodyLen) > p.xmqd.getOpts().MaxMsgSize {
		return nil, protocol.NewFatalClientErr(nil, "E_BAD_MESSAGE",
			fmt.Sprintf("PUB message too big %d > %d", bodyLen, p.xmqd.getOpts().MaxMsgSize))
	}
...
func (p *protocolV2) MPUB(client *clientV2, params [][]byte) ([]byte, error) {
	...
	if int64(bodyLen) > p.xmqd.getOpts().MaxBodySize {
		return nil, protocol.NewFatalClientErr(nil, "E_BAD_BODY",
			fmt.Sprintf("MPUB body too big %d > %d", bodyLen, p.xmqd.getOpts().MaxBodySize))
	}

	messages, err := readMPUB(client.Reader, client.lenSlice, topic,
		p.xmqd.getOpts().MaxMsgSize, p.xmqd.getOpts().MaxBodySize)
	if err != nil {
		return nil, err
	}
...
func (p *protocolV2) DPUB(client *clientV2, params [][]byte) ([]byte, error) {
	...
	if timeoutDuration < 0 || timeoutDuration > p.xmqd.getOpts().MaxReqTimeout {
		return nil, protocol.NewFatalClientErr(nil, "E_INVALID",
			fmt.Sprintf("DPUB timeout %d out of range 0-%d",
				timeoutMs, p.xmqd.getOpts().MaxReqTimeout/time.Millisecond))
	}
	...
	if int64(bodyLen) > p.xmqd.getOpts().MaxMsgSize {
		return nil, protocol.NewFatalClientErr(nil, "E_BAD_MESSAGE",
			fmt.Sprintf("DPUB message too big %d > %d", bodyLen, p.xmqd.getOpts().MaxMsgSize))
	}
...
func enforceTLSPolicy(client *clientV2, p *protocolV2, command []byte) error {
	if p.xmqd.getOpts().TLSRequired != TLSNotRequired && atomic.LoadInt32(&client.TLS) != 1 {
