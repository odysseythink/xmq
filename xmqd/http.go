...
func newHTTPServer(xmqd *XMQD, tlsEnabled bool, tlsRequired bool) *httpServer {
	// log := http_api.Log(xmqd.logf)
	...
	router.Handle("GET", "/stats", http_api.Decorate(s.doStats, http_api.V1))
	...
	router.Handle("POST", "/topic/empty", http_api.Decorate(s.doEmptyTopic, http_api.V1))
	router.Handle("POST", "/topic/pause", http_api.Decorate(s.doPauseTopic, http_api.V1))
	router.Handle("POST", "/topic/unpause", http_api.Decorate(s.doPauseTopic, http_api.V1))
	...
	router.Handle("POST", "/channel/empty", http_api.Decorate(s.doEmptyChannel, http_api.V1))
	router.Handle("POST", "/channel/pause", http_api.Decorate(s.doPauseChannel, http_api.V1))
	router.Handle("POST", "/channel/unpause", http_api.Decorate(s.doPauseChannel, http_api.V1))
	router.Handle("GET", "/config/:opt", http_api.Decorate(s.doConfig, http_api.V1))
	router.Handle("PUT", "/config/:opt", http_api.Decorate(s.doConfig, http_api.V1))
	...
	router.Handle("PUT", "/debug/setblockrate", http_api.Decorate(setBlockRateHandler, http_api.PlainText))
	router.Handle("POST", "/debug/freememory", http_api.Decorate(freeMemory, http_api.PlainText))
...
func (s *httpServer) doInfo(w http.ResponseWriter, req *http.Request, ps httprouter.Params) (interface{}, error) {
	...
	if s.xmqd.RealTCPAddr().Network() == "tcp" {
		tcpPort = s.xmqd.RealTCPAddr().(*net.TCPAddr).Port
	}
	httpPort := -1 // in case of unix socket
	if s.xmqd.RealHTTPAddr().Network() == "tcp" {
		httpPort = s.xmqd.RealHTTPAddr().(*net.TCPAddr).Port
	}
	...
	return struct {
		...
		BroadcastAddress:     s.xmqd.getOpts().BroadcastAddress,
		Hostname:             hostname,
		TCPPort:              tcpPort,
		HTTPPort:             httpPort,
		StartTime:            s.xmqd.GetStartTime().Unix(),
		MaxHeartBeatInterval: s.xmqd.getOpts().MaxHeartbeatInterval,
		MaxOutBufferSize:     s.xmqd.getOpts().MaxOutputBufferSize,
		MaxOutBufferTimeout:  s.xmqd.getOpts().MaxOutputBufferTimeout,
		MaxDeflateLevel:      s.xmqd.getOpts().MaxDeflateLevel,
		Raft: struct {
			State string `json:"state"`
		}{
			State: s.xmqd.cluster.r.State().String(),
		},
	}, nil
}
...
func (s *httpServer) getExistingTopicFromQuery(req *http.Request) (*http_api.ReqParams, *Topic, string, error) {
	...
	if err != nil {
		mlog.Errorf("failed to parse request params - %s", err)
	...
	topic, err := s.xmqd.cluster.GetExistingTopic(topicName)
	if err != nil {
...
func (s *httpServer) getTopicFromQuery(req *http.Request) (url.Values, *Topic, error) {
	...
	return reqParams, s.xmqd.cluster.GetTopic(topicName), nil
}
...
func (s *httpServer) doPUB(w http.ResponseWriter, req *http.Request, ps httprouter.Params) (interface{}, error) {
	...
	if req.ContentLength > s.xmqd.getOpts().MaxMsgSize {
		return nil, http_api.Err{413, "MSG_TOO_BIG"}
	}

	// add 1 so that it's greater than our max when we test for it
	// (LimitReader returns a "fake" EOF)
	readMax := s.xmqd.getOpts().MaxMsgSize + 1
	...
	if ds, ok := reqParams["defer"]; ok {
		...
		if deferred < 0 || deferred > s.xmqd.getOpts().MaxReqTimeout {
			return nil, http_api.Err{400, "INVALID_DEFER"}
		}
	}
...
func (s *httpServer) doMPUB(w http.ResponseWriter, req *http.Request, ps httprouter.Params) (interface{}, error) {
	...
	if req.ContentLength > s.xmqd.getOpts().MaxBodySize {
	...
	if vals, ok := reqParams["binary"]; ok {
		if binaryMode, ok = boolParams[vals[0]]; !ok {
			binaryMode = true
			mlog.Warningf("deprecated value '%s' used for /mpub binary param", vals[0])
		}
	}
	if binaryMode {
		tmp := make([]byte, 4)
		msgs, err = readMPUB(req.Body, tmp, topic,
			s.xmqd.getOpts().MaxMsgSize, s.xmqd.getOpts().MaxBodySize)
		if err != nil {
		...
		// add 1 so that it's greater than our max when we test for it
		// (LimitReader returns a "fake" EOF)
		readMax := s.xmqd.getOpts().MaxBodySize + 1
		...
		for !exit {
			...
			if int64(len(block)) > s.xmqd.getOpts().MaxMsgSize {
				return nil, http_api.Err{413, "MSG_TOO_BIG"}
			}
...
func (s *httpServer) doDeleteTopic(w http.ResponseWriter, req *http.Request, ps httprouter.Params) (interface{}, error) {
	...
	err = s.xmqd.DeleteExistingTopic(topicName)
	if err != nil {
		return nil, http_api.Err{404, "TOPIC_NOT_FOUND"}
	}
...
func (s *httpServer) doPauseTopic(w http.ResponseWriter, req *http.Request, ps httprouter.Params) (interface{}, error) {
	...
	if err != nil {
		mlog.Errorf("failure in %s - %s", req.URL.Path, err)
		return nil, http_api.Err{500, "INTERNAL_ERROR"}
	}

	// pro-actively persist metadata so in case of process failure
	// xmqd won't suddenly (un)pause a topic
	s.xmqd.Lock()
	s.xmqd.PersistMetadata()
	s.xmqd.Unlock()
	return nil, nil
}
...
func (s *httpServer) doPauseChannel(w http.ResponseWriter, req *http.Request, ps httprouter.Params) (interface{}, error) {
	...
	// pro-actively persist metadata so in case of process failure
	// xmqd won't suddenly (un)pause a channel
	s.xmqd.Lock()
	s.xmqd.PersistMetadata()
	s.xmqd.Unlock()
	return nil, nil
...
func (s *httpServer) doStats(w http.ResponseWriter, req *http.Request, ps httprouter.Params) (interface{}, error) {
	...
	stats := s.xmqd.GetStats(topicName, channelName, includeClients)
	health := s.xmqd.GetHealth()
	startTime := s.xmqd.GetStartTime()
...
func (s *httpServer) doConfig(w http.ResponseWriter, req *http.Request, ps httprouter.Params) (interface{}, error) {
	...
	if req.Method == "PUT" {
		...
		opts := *s.xmqd.getOpts()
		switch opt {
		case "xmqlookupd_grpc_addresses":
			err := json.Unmarshal(body, &opts.XMQLookupdGrpcAddresses)
			if err != nil {
		...
		s.xmqd.swapOpts(&opts)
		s.xmqd.triggerOptsNotification()
	}

	v, ok := getOptByCfgName(s.xmqd.getOpts(), opt)
	if !ok {
		return nil, http_api.Err{400, "INVALID_OPTION"}
	}
