...
#
# This script bootstraps an XMQ cluster in EC2 and runs benchmarks.
#
...
def _bootstrap(addr):
    ...
    for cmd in [
            ...
            'mkdir -p go/src/github.com/xmqio',
            'cd go/src/github.com/xmqio && git clone https://mlib.com/xmq',
            'cd go/src/mlib.com/xmq && git checkout %s' % commit,
            'cd go/src/mlib.com/xmq/apps/xmqd && GO111MODULE=on /usr/local/go/bin/go build',
            'cd go/src/mlib.com/xmq/bench/bench_writer && GO111MODULE=on /usr/local/go/bin/go build',
            'cd go/src/mlib.com/xmq/bench/bench_reader && GO111MODULE=on /usr/local/go/bin/go build',
            'sudo -S mkdir -p /mnt/xmq',
            'sudo -S chmod 777 /mnt/xmq']:
...
def bootstrap():
    ...
    total_count = tornado.options.options.xmqd_count + tornado.options.options.worker_count
    ...
    for instance in instances:
        if not instance.tags:
            instance.create_tags(Tags=[{'Key': 'xmq_bench', 'Value': '1'}])
...
def run():
    ...
    logging.info('launching xmqd on %d host(s)', tornado.options.options.xmqd_count)

    xmqd_chans = []
    xmqd_hosts = instances[:tornado.options.options.xmqd_count]
    for instance in xmqd_hosts:
        try:
            ssh_client = ssh_connect_with_retries(instance.public_dns_name)
            for cmd in [
                    'sudo -S pkill -f xmqd',
                    'sudo -S rm -f /mnt/xmq/*.dat',
                    'GOMAXPROCS=32 ./go/src/mlib.com/xmq/apps/xmqd/xmqd \
                        --data-path=/mnt/xmq \
                        --mem-queue-size=10000000 \
                        --max-rdy-count=%s' % (tornado.options.options.rdy)]:
                xmqd_chans.append((ssh_client, ssh_cmd_async(ssh_client, cmd)))
        except Exception:
            logging.exception('failed')

    xmqd_tcp_addrs = [i.public_dns_name for i in xmqd_hosts]

    dt = datetime.datetime.utcnow()
    deadline = dt + datetime.timedelta(seconds=30)

    logging.info('launching %d producer(s) on %d host(s)',
                 tornado.options.options.xmqd_count * tornado.options.options.worker_count,
                 tornado.options.options.worker_count)

    worker_chans = []

    producer_hosts = instances[tornado.options.options.xmqd_count:]
    for instance in producer_hosts:
        for xmqd_tcp_addr in xmqd_tcp_addrs:
            topic = hashlib.md5(instance.public_dns_name.encode('utf-8')).hexdigest()
            try:
                ssh_client = ssh_connect_with_retries(instance.public_dns_name)
                for cmd in [
                        'GOMAXPROCS=2 \
                            ./go/src/mlib.com/xmq/bench/bench_writer/bench_writer \
                            --topic=%s --xmqd-tcp-address=%s:4150 --deadline=\'%s\' --size=%d' % (
                            topic, xmqd_tcp_addr, deadline.strftime('%Y-%m-%d %H:%M:%S'),
    ...
    if tornado.options.options.mode == 'pubsub':
        ...
        consumer_hosts = instances[tornado.options.options.xmqd_count:]
        for instance in consumer_hosts:
            for xmqd_tcp_addr in xmqd_tcp_addrs:
                ...
                try:
                    ...
                    for cmd in [
                            'GOMAXPROCS=8 \
                                ./go/src/mlib.com/xmq/bench/bench_reader/bench_reader \
                                --topic=%s --xmqd-tcp-address=%s:4150 --deadline=\'%s\' --size=%d \
                                --rdy=%d' % (
                                topic, xmqd_tcp_addr, deadline.strftime('%Y-%m-%d %H:%M:%S'),
    ...
    for ssh_client, chan in xmqd_chans:
        chan.close()
...
def _find_instances():
    ...
    return [i for i in ec2.instances.all() if
            i.state['Name'] == 'running' and any(t['Key'] == 'xmq_bench' for t in i.tags)]
...
if __name__ == '__main__':
    ...
    tornado.options.define('xmqd_count', type=int, default=3,
                           help='how many xmqd instances to launch')
